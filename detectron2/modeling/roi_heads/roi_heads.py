...
from .fast_rcnn import FastRCNNOutputLayers, FastRCNNOutputs, FastRCNNOutputLayersV2, FastRCNNOutputsV2
...
class StandardROIHeadsV2(ROIHeads):
    ...
    def __init__(self, cfg, input_shape):
        super(StandardROIHeadsV2, self).__init__(cfg, input_shape)
        self._init_box_head(cfg)
        self._init_mask_head(cfg)
        self._init_keypoint_head(cfg)
    ...
    def _init_box_head(self, cfg):
        ...
        self.box_predictor = FastRCNNOutputLayersV2(
            self.box_head.output_size, self.num_classes, num_classes_aux, self.cls_agnostic_bbox_reg
        )
    ...
    def _forward_box(self, features, proposals):
        ...
        box_features = self.box_pooler(features, [x.proposal_boxes for x in proposals])
        box_features = self.box_head(box_features)
        pred_class_logits_1, pred_class_logits_2,  pred_proposal_deltas = self.box_predictor(box_features)
        del box_features

        outputs = FastRCNNOutputsV2(
            self.box2box_transform,
            pred_class_logits_1,
            pred_class_logits_2,
            pred_proposal_deltas,
